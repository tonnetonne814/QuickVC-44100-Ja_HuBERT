...
def run(rank, n_gpus, hps):
  ...
  try:
    net_g = utils.load_model_diffsize(utils.latest_checkpoint_path(hps.model_dir, "G_*.pth"), net_g, hps, optim_g)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (c, spec, y, f0, uv) in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q) = net_g(c, spec, g=g, mel=mel, f0=f0, uv=uv)
      ...
      if y.shape != y_hat.shape or y_mel.shape != y_hat_mel.shape:
        print("output shape != audio data shape")
