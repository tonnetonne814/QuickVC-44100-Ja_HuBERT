...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        #i = 80#random.randint(68,92)
  
        c_filename = filename.replace(".wav", f".content.npy")
        c = np.load(c_filename)#.squeeze(0)
        c=c.transpose(1,0)
        c = torch.FloatTensor(c)#.squeeze(0)
        
        f0_filename = filename.replace(".wav", ".f0.npy")
        f0 = np.load(f0_filename)
        f0, uv = utils.interpolate_f0(f0)
        
        _,spec_len = spec.shape
        f0_len = f0.shape
        _,c_len = c.shape
        wav_len = audio_norm.shape
        length = max([spec_len, f0_len[0], c_len])

        f0 = torch.FloatTensor(f0)
        uv = torch.FloatTensor(uv)
        
        c = utils.repeat_expand_2d(c.squeeze(0), length)
        """
        if length == spec_len:
            c = utils.repeat_expand_2d(c.squeeze(0), length)
            f0 = utils.repeat_expand_2d(torch.unsqueeze(f0, 0), length)[0]
            uv = utils.repeat_expand_2d(torch.unsqueeze(uv, 0), length)[0]

        elif length == f0_len:
            c = utils.repeat_expand_2d(c.squeeze(0), length)
            spec = utils.repeat_expand_2d(spec, length)
        
        elif length == c_len: 
            spec = utils.repeat_expand_2d(spec.squeeze(0), length)
            f0 = utils.repeat_expand_2d(torch.unsqueeze(f0, 0), length)[0]
            uv = utils.repeat_expand_2d(torch.unsqueeze(uv, 0), length)[0]
        """
        ...
        return c, spec, audio_norm, f0, uv

    def random_slice(self, c, f0, spec, audio_norm, uv):
        # if spec.shape[1] < 30:
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        max_f0_len = max([x[3].size(0) for x in batch])
        max_uv_len = max([x[4].size(0) for x in batch])
        #print(max_spec_len,max_c_len)
        spec_lengths = torch.LongTensor(len(batch))
        wav_lengths = torch.LongTensor(len(batch))
        #f0_lengths = torch.LongTensor(len(batch))
        #uv_lengths = torch.LongTensor(len(batch))
        ...
        # maybe spec f0 uv c length is same
        ...
        #print(spec_padded.size(), spec_lengths, spec_seglen)
        spec_padded, ids_slice = commons.rand_spec_segments(spec_padded, spec_lengths, spec_seglen)
        wav_padded = commons.slice_segments(wav_padded, ids_slice * self.hps.data.hop_length, wav_seglen)[:,:,:-self.hps.data.hop_length]

        c_padded = commons.slice_segments(c_padded, ids_slice, spec_seglen)[:,:,:-1]
        f0_padded = commons.slice_segments_2dim(f0_padded, ids_slice , spec_seglen)[:,:-1]
        uv_padded = commons.slice_segments_2dim(uv_padded, ids_slice , spec_seglen)[:,:-1]
        ...
        return c_padded, spec_padded, wav_padded, f0_padded, uv_padded
