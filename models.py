...
class TextEncoder(nn.Module):
  def __init__(self,
    ...
    self.c_emb = nn.Linear(768, hidden_channels)
  ...
  def forward(self, c, c_lemgth, f0, noice_scale=1):  #c[B, Hid, Frame] f0[B, fra]
    #x = x + self.f0_emb(f0)
    f0_emb = self.f0_emb(f0)
    c = c.transpose(1,2)
    c_emb = self.c_emb(c)
    x = f0_emb + c_emb
    x = self.lrelu(x * math.sqrt(self.hidden_channels)) 
    x = x.transpose(1,2) 
    x_mask = torch.unsqueeze(commons.sequence_mask(c_lemgth, x.size(2)), 1).to(x.dtype)
    #x_mask = commons.sequence_mask(c_lemgth, x.size(2)).to(x.dtype)
...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    #self.enc_p = PosteriorEncoder(768, inter_channels, hidden_channels, 5, 1, 16)#768, inter_channels, hidden_channels, 5, 1, 16)
  ...
  def forward(self, c, spec, g=None, mel=None, f0=None, uv=None, c_lengths=None, spec_lengths=None):
    if c_lengths == None:
    ...
    _, m_p, logs_p, _ = self.enc_p(c, c_lengths, f0=f0_to_coarse(f0))
  ...
  def infer(self, c, g=None, mel=None, c_lengths=None, f0=None, uv=None,):
    if c_lengths == None:
      c_lengths = (torch.ones(c.size(0)) * c.size(-1)).to(c.device)
...
if __name__ == "__main__":
    x = torch.rand(size=(4, 768,500))
    c_len = torch.FloatTensor(torch.rand(4))
    f0 = torch.zeros(size=(4,500),dtype=torch.long)
    ...
    x = enc_p(x, c_len, f0, noice_scale=1)